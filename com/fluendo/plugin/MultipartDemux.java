  ...
  private Pad sinkpad = new Pad(Pad.SINK, "sink") {
    ...
    private void accumulateBuffer (com.fluendo.jst.Buffer buf)
    {
      ...
      if (accum.length < lastPos + buf.length) {
        ...
        /* FIXME, check if we need a bigger buffer or if we can just shrink the
	 * current one */
	...
	lastPos = accumSize;
      ...
      //System.out.println("added "+buf.length+" pos "+accumPos+" size "+accumSize);
    }
    ...
    private void flushBytes (int bytes)
    {
      accumPos += bytes;
      accumSize -= bytes;
      //System.out.println("flushing "+bytes+" pos "+accumPos+" size "+accumSize);
    }

    /* find bytes of consecutive bytes in the buffer. This function
     * returns the position in the buffer where the bytes were found or -1 if the
    ...
    private int findBytes(int startPos, byte[] bytes, int bytesLen) {
	  ...
	  //System.out.println("match: scanPos:"+scanPos+" "+accum[scanPos]+" pos:"+pos+" "+bytes[pos]);
	  /* position to compare the next byte */
          pos++;
	  if (pos == bytesLen) {
	    /* we found all consecutive bytes, we have a complete match */
	    return startPos;
	  }
        }
        else {
	  /* we have no match, flush our buffer to next byte and start scanning for the first
	   * byte in the bytes string again. The data size decrements. */
	  //System.out.println("fail: scanPos:"+scanPos+" "+accum[scanPos]+" pos:"+pos+" "+bytes[pos]);
          scanPos -= pos;
	  size += pos;
	  startPos = scanPos + 1;
	  pos = 0;
     ...
     * returns true if the bytes where found with the accumPos position pointing
     * to the byte in the buffer. */
     ...
     * we store the Content-Type: header in lastContentType */
	...
	String header = new String (accum, headerStart, pos - headerStart);
    ...
    private boolean findDataEnd() {
      int pos = findBytes (accumPos, boundary, boundaryLen);
      //System.out.println("find data end "+accumPos+" size "+accumSize+" pos "+pos);
      if (pos != -1) {
        dataEnd = pos - 1;
      }
      return pos != -1;
